...
public class JWT {
	...
	@Autowired
	private Repository2 tokendb2; // tokendb object of token db class

	@PostMapping("/token")
	public String createJwtToken(@RequestBody UserInformation loginInfoInBody) {
		Optional<DetailsModal> userFound = tokendb2.findById(loginInfoInBody.getUserId());
		if (!userFound.isPresent()) {
			return "UnAuthorized";
		}
		DetailsModal data = userFound.get();
		String role = data.getRole();
		String uname = data.getUserId();
